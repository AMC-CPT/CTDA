...
# external variables: DATA, cID (current subject); the dose 320 is used directly
  ...
  F  <- T3*(T4 - T5)                        # prediction
  G1 <- -F*T2 + KA*TIME*T3*T5               # dF/d eta1
  G2 <- -F                                  # dF/d eta2
  G3 <- F*T2 - K*TIME*T3*T4                 # dF/d eta3
  H1 <- F                                   # dY/d eps1 (proportional error)
  H2 <- 1                                   # dY/d eps2 (additive error)
